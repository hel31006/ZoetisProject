...
def submit_existing():
    ...
    print("\n" + "=" * 50)
    print("[DEBUG] Entering /submit_existing route")
    print(f"[DEBUG] Total records to process from review page (count): {count}")
    print(f"[DEBUG] Initial new clients from hidden JSON field: {new_clients_from_upload}")
    print("=" * 50 + "\n")
    ...
    with get_connection() as connection:
        for i in range(count):
            ...
            print(f"[DEBUG] Processing record index {i} from review page...")
            print(f"[DEBUG] Decision for record {i}: {decision}")
            ...
            if decision == "new":
                print(f"[DEBUG] Record {i} marked as NEW. Appending to confirmed_new_clients list.")
                print(f"[DEBUG] Data being appended: {record_data}")
    ...
    print("\n" + "-" * 50)
    print(f"[DEBUG] Loop finished. confirmed_new_clients list is now: {confirmed_new_clients}")

    final_new_clients = new_clients_from_upload + confirmed_new_clients
    print(f"[DEBUG] Combined list (final_new_clients) before deduplication: {final_new_clients}")
    ...
    print(f"[DEBUG] Final unique list being passed to bulk template: {unique_new_clients}")
    print("=" * 50 + "\n")
    # ==================================================================
...
def submit_new_clinics():
    ...
    with get_connection() as connection:
        for i in range(count):
            # ... (rest of the logic for inserting new clinics)
